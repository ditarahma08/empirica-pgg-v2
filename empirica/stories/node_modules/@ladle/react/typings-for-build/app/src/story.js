...
const StoryFrame = ({ children, active, width, darkTheme, story, mode, }) => {
    if ((!active && width === 0) || mode === ModeState.Preview)
        return children;
    return (_jsx(Frame, { title: `Story ${story}`, initialContent: `<!DOCTYPE html><html><head><base target="_parent" /></head><body style="margin:0"><div id="root"></div></body></html>`, mountTarget: "#root", style: {
...
const SynchronizeHead = ({ active, children, rtl, }) => {
...
const Story = ({ globalState, dispatch, }) => {
    ...
    return (_jsx(ErrorBoundary, { children: _jsx(React.Suspense, { fallback: _jsx(Ring, {}), children: _jsx(StoryFrame, { active: iframeActive, story: globalState.story, width: width, mode: globalState.mode, darkTheme: globalState.theme === ThemeState.Dark, children: _jsx(SynchronizeHead, { active: (iframeActive || width > 0) &&
                        globalState.mode !== ModeState.Preview, rtl: globalState.rtl, children: _jsx(MDXProvider, { components: {
                            code: (props) => (_jsx(CodeHighlight, { ...props, theme: globalState.theme })),
                        }, children: _jsx(Provider, { config: config, globalState: globalState, dispatch: dispatch, storyMeta: storyDataMeta, children: storyData ? (React.createElement(storyData.component)) : (_jsx(NoStories, { wrongUrl: true, activeStory: globalState.story })) }) }) }) }) }) }));
};
